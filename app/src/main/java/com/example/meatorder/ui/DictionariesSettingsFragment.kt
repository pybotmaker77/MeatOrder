...
class DictionariesSettingsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Инициализация dao и prefs, когда фрагмент уже присоединён
        dao = getDao()
        prefs = getPrefs()
    ...
    private fun exportFiles() {
        lifecycleScope.launch {
            try {
                // Получаем данные ДО создания архива, чтобы не было проблем с контекстом
                ...
                // Отправка через системный share intent
    ...
    // Вспомогательные методы импорта (уже были, но повторю для полноты)
